               ...
               <div className="w-full lg:-mx-4 lg:w-[calc(100%+2rem)] bg-slate-50 border-t border-b border-slate-200 p-4 lg:p-6 shadow-inner relative animate-in slide-in-from-top-2 duration-300 rounded-b-3xl lg:rounded-b-2xl mb-4">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                     ...
                     <button onClick={() => {
                        const updatedProps = properties.map(p => {
                          if (p.id === prop.id) {
                            return { ...p, status: prop.status.includes('مسودة') || prop.status.includes('منتهي') ? 'نشط' : 'منتهي / مسودة' };
                          }
                          return p;
                        });
                        setProperties(updatedProps);
                     }} className="flex items-center gap-2 px-3 py-2 text-sm font-bold text-slate-700 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors">
                        <Archive className="w-4 h-4" /> {prop.status.includes('مسودة') || prop.status.includes('منتهي') ? 'تنشيط العقار' : 'أرشفة مسودة'}
                     </button>
                                  ...
                                  prop.issues?.map(issue => (
                                    <div key={issue.id} className="flex justify-between items-center bg-amber-50 p-3 rounded-xl border border-amber-100">
                                      ...
                                      <div className="flex gap-2">
                                        ...
                                        <button onClick={() => {
                                            setProperties(properties.map(p => p.id === prop.id ? {...p, issues: p.issues?.map(i => i.id === issue.id ? {...i, status: issue.status === 'مفتوحة' ? 'مغلقة' : 'مفتوحة'} : i)} : p));
                                          }} className="px-2 py-1 bg-white border border-slate-200 rounded-md text-[10px] font-bold hover:bg-slate-50 transition-colors">{issue.status === 'مفتوحة' ? 'إغلاق' : 'إعادة فتح'}</button>
                                      </div>
                                    </div>
                                  ))
                                )}
