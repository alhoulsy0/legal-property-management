...
import { ArrowLeft, Building, Plus, FileText, DollarSign, MapPin, UploadCloud, User, Calendar as CalendarIcon, Clock } from "lucide-react";
...
export default function ClientProfilePage() {
  ...
  const [properties, setProperties] = useState([
    { id: 1, type: "Residential", name: "Sunset Apartments", location: "123 Sunset Blvd", tenant: "Alice Johnson", status: "Active", paymentFreq: "Monthly (1st)", revenue: 5000 },
    { id: 2, type: "Commercial", name: "Downtown Office Plaza", location: "456 Main St", tenant: "TechCorp Inc.", status: "Delayed", paymentFreq: "Yearly", revenue: 10000 },
  ]);
  ...
  const handleAddProperty = (e: React.FormEvent) => {
    ...
    if (newPropName && newPropLocation) {
      ...
      setIsAdding(false);
      // Reset
      setNewPropName(""); setNewPropType("Residential"); setNewPropLocation(""); setNewPropTenant(""); setNewPropFreq("Monthly (1st)"); setNewPropStatus("Active"); setNewPropRev("");
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <button
          ...
          className="bg-slate-900 text-white px-5 py-2.5 rounded-xl shadow-sm hover:bg-slate-800 transition-colors font-semibold flex items-center space-x-2 text-sm"
        >
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 mb-6 relative overflow-hidden">
          <h3 className="text-lg font-bold mb-5 text-slate-900 border-b pb-3">Register New Property</h3>
          <form onSubmit={handleAddProperty} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              <div className="col-span-full">
                ...
                <div className="w-full border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer group">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        {properties.map((prop) => (
          <div key={prop.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between">
            <div>
